...
void main() {
  group('formatCurrency', () {
    test('formats USD correctly in en_US locale', () {
      final result = FormatUtils.formatCurrency(Currency.usd, 1234.56, 'en_US');
      expect(result, '\$1,234.56');
    });

    test('formats EUR correctly in es_ES locale', () {
      final result = FormatUtils.formatCurrency(Currency.eur, 1234.56, 'es_ES');
      expect(result, '1.234,56 €');
    });

    test('formats GBP correctly in en_GB locale', () {
      final result = FormatUtils.formatCurrency(Currency.gbp, 1234.56, 'en_GB');
      expect(result, '£1,234.56');
    });

    test('formats JPY correctly in ja_JP locale', () {
      final result = FormatUtils.formatCurrency(Currency.jpy, 1234, 'ja_JP');
      expect(result, '¥1,234.00');
    });

    test('formats CAD correctly in en_CA locale', () {
      final result = FormatUtils.formatCurrency(Currency.cad, 1234.56, 'en_CA');
      expect(result, 'C\$1,234.56');
    });

    test('formats AUD correctly in en_AU locale', () {
      final result = FormatUtils.formatCurrency(Currency.aud, 1234.56, 'en_AU');
